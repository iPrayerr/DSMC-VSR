...
if __name__ == '__main__':
    ...
    trainset = VimeoTrainSetLoader(trainset_dir_hr=opt.trainhr, trainset_dir_lr=opt.trainlr,
    ...
    valset = VimeoValSetLoader(valset_dir_hr=opt.valhr, valset_dir_lr=opt.vallr,
                                upscale_factor=4, patch_size=opt.patch_size)
    ...
    for epoch in range(opt.epochs):
        ...
        PSNR_Avg = 0.0
        SSIM_Avg = 0.0
        for i,data in enumerate(tqdm(trainloader,desc="Epoch %d/%d"%(epoch+1, opt.epochs), ncols=100),0):
            optimizer.zero_grad()
            l,gt = data
        ...
        if epoch%1 == 0:
            ...
            SSIM_Avg = 0.0
            Sbic_Avg = 0.0
            for j, valdata in enumerate(tqdm(validloader,desc="Validation", ncols=100), 0):
                vl, vgt = valdata
